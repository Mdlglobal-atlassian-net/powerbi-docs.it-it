...
In questo frammento di codice si presuppone che il set di dati di cui è possibile eseguire il push si trovi già in un'area di lavoro all'interno dell'ambiente SaaS. Per informazioni sull'API push, vedere [Eseguire il push dei dati in un set di dati di Power BI](../automation/walkthrough-push-data.md).

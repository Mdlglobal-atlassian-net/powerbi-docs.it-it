...
Quando si usano le [API di Power BI](https://docs.microsoft.com/rest/api/power-bi/), assicurarsi di definire le operazioni usando l'[ID oggetto entità servizio](embed-service-principal.md) per fare riferimento all'entità servizio, ad esempio applicando un'entità servizio come amministratore a un'area di lavoro.
...
4. Selezionare le autorizzazioni specifiche necessarie in **Autorizzazioni delegate**. Per salvare le selezioni, selezionarle una alla volta. Al termine, selezionare **Salva**.

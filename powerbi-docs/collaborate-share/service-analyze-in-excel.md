...
## <a name="two-ways-to-get-started"></a>Due modi per iniziare
Esistono due modi diversi per esplorare i set di dati di Power BI in Excel. Se si inizia da Power BI, seguire i passaggi descritti in questo documento.  È ora disponibile anche la possibilità per gli utenti con determinati SKU di Office di accedere ai set di dati direttamente dall'esperienza Recupera dati all'interno di una cartella di lavoro di Excel.  È possibile esplorare i set di dati a cui si ha accesso, verificare se i set di dati sono certificati o promossi e se sono state applicate etichette di protezione dati.  Per altre informazioni su questa esperienza, vedere [Creare una tabella pivot da set di dati di Power BI](https://support.office.com/article/31444a04-9c38-4dd7-9a45-22848c666884) nella documentazione di Excel.
...
* Le tabelle pivot di Excel non supportano l'aggregazione di campi numerici mediante trascinamento della selezione. Il set di dati in Power BI *deve avere misure predefinite*. Vedere le informazioni sulla [creazione di misure](../transform-model/desktop-measures.md).
...
* Gli utenti possono connettersi ai set di dati tramite Analizza in Excel se hanno l'autorizzazione per il set di dati sottostante.  Un utente può ottenere questa autorizzazione in diversi modi, ad esempio perché ha il ruolo Membro nell'area di lavoro che contiene il set di dati, perché condivide un report o un dashboard che usa il set di dati o perché dispone dell'autorizzazione per creazione report per il set di dati, in un'area di lavoro o un'app contenente il set di dati. Per altre informazioni, vedere [Autorizzazione per creazione report](../connect-data/service-datasets-build-permissions.md) per i set di dati.
* Gli utenti guest non possono usare **Analizza in Excel** per i set di dati inviati da un altro tenant. 
...
Quando si seleziona **Analizza in Excel** dal menu **Altre opzioni** (...) insieme a un set di dati o a un report di **Power BI**, Power BI crea un file con estensione odc e lo scarica dal browser nel computer.

![Analizza in Excel](media/service-analyze-in-excel/power-bi-analyze-in-excel.png)
...
## <a name="get-started-with-analyze-in-excel-in-power-bi"></a>Introduzione ad Analizza in Excel in Power BI
In Power BI selezionare il menu **Altre opzioni** accanto a un report o un set di dati (i puntini di sospensione accanto al nome del report o del set di dati) e selezionare **Analizza in Excel** dal menu visualizzato.

![Analizza in Excel](media/service-analyze-in-excel/power-bi-analyze-menu.png)
...
![Casella di controllo Non visualizzare più questo messaggio](media/service-analyze-in-excel/pbi_anlz_excel_dontshow.png)
...
![Installare gli aggiornamenti](media/service-analyze-in-excel/pbi_anlz_excel_download_again.png)
...
![Abilitare le connessioni dati](media/service-analyze-in-excel/pbi_anlz_excel_enable.png)
...
![Tabella e grafico pivot in Excel](media/service-analyze-in-excel/pbi_anlz_excel_chart.png)
...
* [Usare il drill-through tra report in Power BI Desktop](../create-reports/desktop-cross-report-drill-through.md)

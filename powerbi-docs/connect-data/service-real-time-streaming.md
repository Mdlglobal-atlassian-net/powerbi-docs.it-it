...
Con un set di dati di streaming **PubNub** il client Web di Power BI usa l'SDK PubNub per leggere un flusso di dati PubNub e nessun dato viene memorizzato dal servizio Power BI. Poiché questa chiamata viene effettuata direttamente dal client Web, è necessario aggiungere all'elenco elementi consentiti il traffico a PubNub se si autorizza solo il traffico consentito in uscita dalla rete. Vedere le istruzioni nell'articolo relativo al supporto per [l'aggiunta del traffico in uscita all'elenco elementi consentiti per PubNub](https://support.pubnub.com/support/solutions/articles/14000043522-can-i-whitelist-ips-for-pubnub-traffic-).

...
Questa esercitazione illustra i passaggi necessari per implementare la **sicurezza a livello di riga** all'interno di un **modello tabulare di Analysis Services** e mostra come usarlo in un report di Power BI. I passaggi descritti in questa esercitazione sono progettati per poter proseguire e apprendere i passaggi necessari per completare un set di dati di esempio.
...
Per seguire i passaggi in questa esercitazione è necessario il database **AdventureworksDW2012**, che è possibile scaricare **[qui.](http://msftdbprodsamples.codeplex.com/releases/view/55330)**
...
2. Dopo aver creato e salvato la tabella, è necessario creare la relazione tra la colonna **SalesTerritoryID** della tabella **DimUserSecurity** e la colonna **SalesTerritoryKey** della tabella **DimSalesTerritory**colonna, come illustrato nell'immagine seguente. Questa operazione può essere eseguita da **SSMS** facendo clic con il pulsante destro del mouse sulla tabella **DimUserSecurity** e scegliendo **Modifica**.
...
1. Dopo aver creato il data warehouse relazionale, è necessario definire il modello tabulare. Il modello può essere creato usando **SQL Server Data Tools (SSDT)**. Per altre informazioni su come definire un modello tabulare, [consultare questo articolo](https://msdn.microsoft.com/library/hh231689.aspx).
2. Importare tutte le tabelle necessarie nel modello come mostrato di seguito.
...
7. In questa formula la funzione **LOOKUPVALUE** restituisce tutti i valori per la colonna **DimUserSecurity [SalesTerritoryID]**, dove **DimUserSecurity [UserName]** è uguale al nome utente di Windows connesso e **DimUserSecurity [SalesTerritoryID]** è uguale a **DimSalesTerritory [SalesTerritoryKey]**.
...
8. Per la tabella **DimUserSecurity**, nella colonna **Filtro DAX** digitare la formula seguente.
   
       =FALSE()
9. Questa formula specifica che tutte le colonne vengono risolte nella condizione booleana false, quindi non è possibile eseguire query nelle colonne della tabella **DimUserSecurity**.
10. A questo punto è necessario elaborare e distribuire il modello. Per assistenza sulla distribuzione del modello è possibile vedere [questo articolo](https://msdn.microsoft.com/library/hh231693.aspx).
...
1. Una volta che il modello tabulare è distribuito e pronto per l'utilizzo, è necessario aggiungere una connessione origine dati al server tabulare di Analysis Services locale con il portale di Power BI.
2. Per consentire al **servizio Power BI** di accedere all'istanza di Analysis Services locale, è necessario che il **[gateway dati locale](service-gateway-onprem.md)** sia installato e configurato nell'ambiente.
3. Dopo aver configurato correttamente il gateway, è necessario creare una connessione all'origine dati per l'istanza tabulare di **Analysis Services**. Questo articolo descrive come [aggiungere un'origine dati con il portale di Power BI](service-gateway-enterprise-manage-ssas.md).
...
4. Si noterà che il modello che è stato distribuito nell'istanza di **Analysis Services**. Selezionare il modello corrispondente, quindi selezionare **OK**.
...
1. Dopo aver creato il report e fatto clic su **Pubblica** in **Power BI Desktop**, il report viene pubblicato nel servizio **Power BI**. Ora che è incluso nel servizio, è possibile fornire una dimostrazione di questo scenario di sicurezza del modello usando l'esempio creato nei passaggi precedenti.
   ...
   Dopo aver pubblicato il report, nel servizio Power BI crea un dashboard denominato **TabularDynamicSec** basato su tale report. Nell'immagine seguente si noti che il responsabile vendite (Sumit) può visualizzare i dati corrispondenti a tutte le aree di vendita.
   
   ![](media/desktop-tutorial-row-level-security-onprem-ssas-tabular/donut_chart_1.png)
2. Ora Sumit condivide il dashboard con colleghi, Jon Doe, Responsabile vendite nell'area Australia.
...
3. In base alla richiesta del nome utente effettivo, Analysis Services converte la richiesta nella credenziale moonneo\jondoe effettiva dopo l'esecuzione di query nell'istanza di Active Directory locale. Una volta che **Analysis Services** ottiene la credenziale effettiva da Active Directory, in base alle autorizzazioni di accesso ai dati dell'utente, **Analysis Services** restituisce solo i dati per i quale l'utente non ha l'autorizzazione.
...
2. Le eventuali modifiche apportate ai dati dopo l'elaborazione del modello saranno immediatamente disponibili per gli utenti che accedono al report in base alla **connessione dinamica** del servizio Power BI.
